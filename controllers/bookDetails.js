...
const getAllDetails = async (req,res) => {
    ...
    try {
        ...
        console.log(error);
        res.status(500).json({mensagem:'Could not find the data.'})
    }
...
const updateDetail = async (req, res) => {
    try {
      ...
      if (updateDetail) {
        res.status(200).json({ mensagem: 'The update was successful.' });
      } else {
        res.status(404).json({ mensagem: 'There is no item to update.' });
      }
    } catch (error) {
      console.log('Error:', error);
      res.status(500).json({ mensagem: 'Something went wrong.' });
    }
  };
...
const deleteDetail = async (req, res) => {
    try {
        ...
        if (deleteDetail) {
            res.status(200).json({ mensagem: 'The removal was successful.' });
        } else {
            res.status(300).json({ mensagem: 'There is no item to remove.' });
        }
    } catch (error) {
        res.status(500).json({ mensagem: 'Something went wrong.' });
    }
